...
public class RegistrationComplete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //インスタンス取得
            UserData udr =(UserData)hs.getAttribute("ud");
            
            //DTOオブジェクトにマッピング。DB専用のパラメーターに変換
            UserDataDTO udto = new UserDataDTO();
            udr.UD2DTOMapping(udto);
            
            //DBへデータの挿入(insert)
            UserDataDAO .getInstance().insert(udto);
            
            //DBへ格納したらセッション破棄
            hs.invalidate();
            
            //結果画面表示用に入力パラメーターをリクエストスコープに格納
            request.setAttribute("udr",udr);
